...
export default function CheckoutPage() {
  ...
  if (!draft) {
    return (
      <section className="section-card rounded-3xl bg-quiz-panel text-white">
        <p className="type-body-sm text-white/80">No item selected. Go back to the shop or wallet.</p>
        <button type="button" onClick={leaveCheckout} className="mt-4 type-body-sm text-[#93c5fd] underline">
          Back
        </button>
      </section>
    )
  ...
  return (
    <section className="section-card !overflow-visible rounded-3xl bg-quiz-panel text-white">
      <div className="mx-auto w-full max-w-xl space-y-6 px-1 sm:space-y-8 sm:px-0">
        {/* Selection Highlight Header */}
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-4 shadow-2xl sm:p-6">
          ...
          <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-5">
              {draft.iconUrl ? (
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20 p-3 shadow-xl ring-1 ring-white/10 sm:h-20 sm:w-20">
                  <img src={draft.iconUrl} alt="" className="h-full w-full object-contain drop-shadow-glow" />
                </div>
              ) : (
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-3xl sm:h-20 sm:w-20 sm:text-4xl">
                  🎁
                </div>
              )}
              <div className="space-y-1">
                <p className="type-caption font-black uppercase tracking-[0.2em] text-[#ffd66b]">Your Selection</p>
                <h2 className="type-section-title font-black tracking-tight text-white text-safe">
                  {draft.label}
                </h2>
                {draft.price != null && (
                  <p className="type-card-title font-bold text-white/90 text-safe">
            ...
            <button
              ...
              className="group flex items-center gap-2 self-start rounded-full bg-white/10 px-4 py-2 text-fluid-sm font-bold text-white transition hover:bg-white/20 active:scale-95 sm:self-auto sm:px-5"
            >
        ...
        {!hasToken ? (
          <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 type-body-sm text-amber-100">
            Sign in is required for card and PayPal checkout.
          </div>
        ) : null}

        {paypalMsg ? (
          <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 type-body-sm text-emerald-100">
            {paypalMsg}
          </div>
        ) : null}

        {isSub ? (
          <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-3 type-body-sm leading-relaxed text-white/70 sm:px-4">
            <p>
        ...
        {isSub ? (
          <div className="space-y-6">
            {checkoutError ? (
              <div className="rounded-xl border border-red-500/35 bg-red-500/10 px-4 py-3 type-body-sm text-red-100">
                {checkoutError}
              </div>
            ) : null}
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <h3 className="type-caption font-semibold uppercase tracking-wide text-white/60">Stripe</h3>
              <p className="mt-2 type-body-sm text-white/70">Secure hosted Checkout — subscriptions supported.</p>
              <button
                type="button"
                disabled={stripeBusy || !hasToken}
                onClick={() => void payStripe()}
                className="mt-4 w-full rounded-xl bg-[#635bff] px-4 py-3 type-body-sm font-semibold text-white transition hover:bg-[#5349e0] disabled:opacity-50"
              >
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <h3 className="type-caption font-semibold uppercase tracking-wide text-white/60">PayPal</h3>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <h3 className="type-caption font-semibold uppercase tracking-wide text-white/60">Stripe</h3>
              <p className="mt-2 type-body-sm text-white/70">You will complete payment on Stripe&apos;s secure page.</p>
              <button
                type="button"
                disabled={stripeBusy || !hasToken}
                onClick={() => void payStripe()}
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <h3 className="type-caption font-semibold uppercase tracking-wide text-white/60">PayPal</h3>
              <p className="mt-2 type-body-sm text-white/70">Pay with PayPal (balance or linked card).</p>
              <div className="mt-4">
        ...
        <p className="text-fluid-xs leading-relaxed text-white/45">
